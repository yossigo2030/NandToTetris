...
class Parser:
    ...
    input code. It reads VMtranslator commands, parses them, and provides convenient
    ...
    def command_type(self) -> str:
        """
            Returns:
            str: the type of the current VMtranslator command.
        ...
        if self.currentCommend.startswith("pop"):
            return "C_POP"
        if self.currentCommend.startswith("label"):
            return "LABEL"
        if self.currentCommend.startswith("goto"):
            return "GOTO"
        if self.currentCommend.startswith("if-goto"):
            return "IF"
        if self.currentCommend.startswith("function"):
            return "FUNCTION"
        if self.currentCommend.startswith("return"):
            return "RETURN"
        if self.currentCommend.startswith("call"):
            return "CALL"
        else:
            return "EX8"
    ...
    def arg2(self) -> int:
        """
         ...
         """
        split = self.currentCommend.split(" ")
        return int(split[1])
